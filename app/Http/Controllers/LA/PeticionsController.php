...
class PeticionsController extends Controller
{
	...
	public function index()
	{
		...
		if(Module::hasAccess($module->id)) {
			return View('la.peticions.index', [
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laraadmin.adminRoute') . '/peticions/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Peticions", "edit")) {
					$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/peticions/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
				
				if(Module::hasAccess("Peticions", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.peticions.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
